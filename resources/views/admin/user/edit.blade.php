                        ...
                        <div class="card-body">
                            <form action="{{ route('users.update', ['id' => $user->id]) }}" method="post" enctype="multipart/form-data">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="password" class="form-control @error('name') is-invalid @enderror" placeholder="Nhập password" value="">
                                    @error('password')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="role_id[]" class="form-control select2_init" multiple>
                                        ...
                                        <option value="{{ $role->id }} " {{ $roleOfUser->contains('id', $role->id) ? 'selected' : '' }}>{{ $role->display_name }}</option>
                                        @endforeach
